"""YouTube Learning Tracker — Streamlit web app (v0.10.0)."""
...
with st.sidebar:
    ...
    if st.session_state["_last_page"] != page:
        ...
        st.session_state["_last_page"] = page
    st.divider()
    # fix(B14): counts assigned before any page block that references it
...
# ── Library ────────────────────────────────────────────────────────────────
    ...
    # fix(B9): single storage read; all filters operate on this list
    all_videos: list[Video] = (
        storage.get_all_videos() if status_filter == "All"
        else storage.filter_by_status(WatchStatus(status_filter))
    )

    # fix(B1/B6): channel + tag pools built from status-filtered list, not all videos
    ...
    all_tags: list[str] = sorted({tag for v in all_videos for tag in (v.tags or [])})
    ...
    if search_q:
        ...
        videos = [v for v in videos if q_lower in v.title.lower() or q_lower in (v.channel or "").lower()]
    if selected_channels:
        videos = [v for v in videos if v.channel in selected_channels]
    if selected_tags:
        videos = [v for v in videos if all(tag in (v.tags or []) for tag in selected_tags)]
    ...
    if sort_by == "Title A–Z":
        ...
        videos = sorted(videos, key=lambda v: v.created_at or "", reverse=True)
    elif sort_by == "Date Added ↑":
        videos = sorted(videos, key=lambda v: v.created_at or "")
    else:
        ...
        if coll:
            ...
            with st.expander("➕ Add videos to this collection"):
                ...
                if not not_in:
                    ...
                    filtered  = (
                        [v for v in not_in if search_q2.lower() in v.title.lower() or search_q2.lower() in v.channel.lower()]
                        if search_q2 else not_in
                    )
...
# ── Settings ───────────────────────────────────────────────────────────────
    ...
    # ── §2  E1: Export full library JSON ──────────────────────────────────
    st.divider()
    st.subheader("⬇️ Export Library")
    st.caption("Download a complete backup of all your videos, notes, progress, tags, and collections.")

    ex_col1, ex_col2, ex_col3 = st.columns(3)

    with ex_col1:
        st.markdown("**📦 JSON (full backup)**")
        st.caption("All videos + collections. Import-compatible.")
        ...
        st.download_button(
            label="⬇️ Export JSON",
            data=json_bytes,
            file_name=f"yt-tracker-backup-{export_ts}.json",
            mime="application/json",
    ...
    with ex_col2:
        ...
        st.caption("16-column table — title, status, progress, tags, notes, and more.")
        all_vids_ex = storage.get_all_videos()
        csv_text    = export_csv(all_vids_ex)
        st.download_button(
            label="⬇️ Export CSV",
            data=csv_text.encode("utf-8"),
            file_name=f"yt-tracker-{export_ts}.csv",
            mime="text/csv",
            key="export_csv",
        )

    with ex_col3:
        st.markdown("**📝 Markdown (readable)**")
        st.caption("Full library grouped by status — shareable & human-readable.")
        all_colls_ex = storage.get_all_collections()
        md_text      = export_markdown_library(all_vids_ex, all_colls_ex)
        st.download_button(
            label="⬇️ Export Markdown",
            data=md_text.encode("utf-8"),
            file_name=f"yt-library-{export_ts}.md",
            mime="text/markdown",
            key="export_md",
        )
    ...
    # ── §3  E2: Import JSON ───────────────────────────────────────────────
    st.divider()
    st.subheader("⬆️ Import Library")
    st.caption("Restore from a previously exported JSON backup, or sync from another machine.")

    import_mode = st.radio(
        "Import mode",
        ["🔀 Merge (add new IDs only — safe)", "♻️ Overwrite (replace entire library)"],
        key="import_mode_radio",
        help="Merge skips any IDs already present. Overwrite replaces everything.",
    )
    merge_flag = import_mode.startswith("🔀")

    uploaded_file = st.file_uploader(
        "Choose a .json backup file",
        type=["json"],
        key="import_json_uploader",
        help="Must be a file previously exported by this app (schema_version: 1).",
    )
    ...
    if uploaded_file is not None:
        ...
        if raw_payload is not None:
            schema_ver = raw_payload.get("schema_version")
            if schema_ver != 1:
                st.warning(
                    f"⚠️ Unknown schema_version `{schema_ver}`. "
                    "This file may have been exported from a different version. Proceed with caution."
                )
            n_videos_in = len(raw_payload.get("videos", {}))
            n_colls_in  = len(raw_payload.get("collections", {}))
            exported_at = raw_payload.get("exported_at", "unknown")
            st.info(
                f"📄 **File summary:** {n_videos_in} video(s), {n_colls_in} collection(s)  |  "
                f"Exported: `{exported_at}`  |  Mode: **{'Merge' if merge_flag else 'Overwrite'}**"
            )

            action_label = "🔀 Merge Import" if merge_flag else "♻️ Overwrite Import"
            if not merge_flag:
                st.error("⚠️ **Overwrite mode** will permanently delete your current library. This cannot be undone.")

            if st.button(action_label, key="do_import_btn", type="primary"):
                try:
                    n_v, n_c = storage.import_json(raw_payload, merge=merge_flag)
                    if merge_flag:
                        st.success(f"✅ Merge complete — {n_v} new video(s) and {n_c} new collection(s) added.")
                    else:
                        st.success(f"✅ Overwrite complete — library replaced with {n_v} video(s) and {n_c} collection(s).")
                    st.rerun()
    ...
    if st.session_state["ytdlp_update_log"]:
        if st.session_state["ytdlp_update_done"]:
            last_line = [ln for ln in st.session_state["ytdlp_update_log"].splitlines() if ln.strip()][-1]
    ...
    st.caption("Permanently delete all videos from your local library. This cannot be undone.")
    if "clear_armed" not in st.session_state:
        st.session_state["clear_armed"] = False
    if st.button("🗑️ Clear All Videos", key="clear_all_btn"):
        st.session_state["clear_armed"] = True
        st.rerun()
    if st.session_state["clear_armed"]:
        st.error("⚠️ This will permanently delete **all videos and their data** from your library. Collections will be emptied but not deleted.")
        dz_col1, dz_col2 = st.columns(2)
        with dz_col1:
            if st.button("✅ Yes, delete everything", key="clear_confirm_btn", type="primary"):
